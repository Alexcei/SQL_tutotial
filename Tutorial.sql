...
Реляционные бд (MySQL) - состоят из друхмерных таблиц
Нереляционные бд (Mongo) - не существует понятия отношений, кортежей и атрибутов.
Строки - записи
Столбцы - поля
Первичный ключ - ID
SQL - яп
...
Ограничения (Constraints) - ограничения на тип данных таблицы.
...
NULL - запись отсутствует (не 0! или _)
...
- Вторая нормальная форма (2NF) — каждое неключевое значение столбца зависит от первичного ключа
- Третья нормальная форма (3NF) — 2NF + и не имеет зависимости от другого неключевого значения столбца
